jQuery(document).ready(function($) {
	...
	$('#he-form-ayarlar').submit(function(event){
		...
		postveri($('#he-form-ayarlar').serialize(), function(verib){
			if (verib == "RES"){
	...
	$('#he-form-ke').submit(function(event){
		...
		postveri($('#he-form-ke').serialize(), function(verib){
			verib = verib.trim();
			if (verib == "RES"){
				window.location.reload();
			}else {
	...
	$('#he-form').submit(function( event  ){
		...
		postveri($('#he-form').serialize()+"&ajanslar_str="+ajanslar_str+"&kategoriler_str="+kategoriler_str, function(verib){
			if (verib == "RES"){
				window.location.reload();
			} else {
				$(".ajaxMsg").html(verib);
	...
	$('#he-form-register').submit(function(event){
		...
		postveri($('#he-form-register').serialize(), function(verib){
			verib = verib.trim();
			if (verib == "RES"){
				window.location.reload();
			}else {
   ...
   function postveri(veri, callback) {
		$.ajax({
			type: "POST",
			url: "/wp-content/plugins/habereditoru/query.php",
			data: veri,
...
$.ajax({type:'POST',url:"../query.php",data:$('#'+form).serialize()})
	.done(function(data) {
		$('#' + to).html(data) ;
	})
	.fail(function() {
...
if ( console && console.log ){console.log(form + "--" + to);}
$('#' + to).html('<img src="../img/loading.gif" />');
$.ajax({type:'POST',url:"../query.php?"+form})
	.done(function(data) {
		$('#' + to).html(data) ;
	})
	.fail(function() {
